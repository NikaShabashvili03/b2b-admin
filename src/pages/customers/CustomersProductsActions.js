...
const Products = () => {
  const [productsData, setProductsData] = useState([
    {
      ...
      isNew: true,
      date: "2024-08-01",
      percentage: 10, // Add percentage state here
    },
    {
      id: 2,
      serialNumber: "IPC3624LE-ADF28K-WL",
      ...
      isNew: false,
      date: "2024-09-14",
      percentage: 15, // Add percentage state here
    }
  ]);
  ...
  const handlePercentageChange = (productId, event) => {
    const updatedProducts = productsData.map(product =>
      product.id === productId ? { ...product, percentage: event.target.value } : product
    );
      ...
      <div className="grid grid-cols-1 2xl:grid-cols-4 xl:grid-cols-4 lg:grid-cols-3 md:grid-cols-2 gap-6">
        {productsData.map((product) => (
          <div key={product.id} className="border p-4 rounded-lg shadow-lg relative">
            {product.percentage && (
              <div className="absolute top-2 right-2 text-white bg-black px-2 py-1 text-sm font-semibold rounded-md">
                {product.percentage}%
              </div>
            )}
            ...
            {checkedProducts.some(item => item.id === product.id) && (
              <div className="mt-4">
                <input
                  ...
                  onChange={(e) => handlePercentageChange(product.id, e)}
                />
                <button
                  className="bg-blue-600 text-white px-4 py-2 rounded-md"
                  disabled={!product.percentage} // Button will be enabled when there's input
                >
                  Save
                </button>
              </div>
